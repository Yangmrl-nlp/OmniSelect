...
class Qwen2_5OmniVisionFlashAttention2(nn.Module):
    ...
    def forward(
        self, hidden_states: torch.Tensor, cu_seqlens: torch.Tensor, rotary_pos_emb: torch.Tensor = None,return_logits: bool = False
    ) -> torch.Tensor:
        seq_length = hidden_states.shape[0]
        q = self.q(hidden_states).reshape(seq_length, self.num_heads, -1)
        k = self.k(hidden_states).reshape(seq_length, self.num_heads, -1)
...
class Qwen2_5OmniVisionBlock(nn.Module):
    ...
    def forward(self, hidden_states, cu_seqlens, rotary_pos_emb,return_logits: bool = False,) -> torch.Tensor:
        hidden_states_tmp = self.attn(
...
class Qwen2_5OmniVisionEncoder(Qwen2_5OmniPreTrainedModel):
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
        ...
        T_feat, H_feat, W_feat = grid_thw[0].tolist()
        window_index, cu_window_seqlens = self.get_window_index(grid_thw)
        window_index_before,_ = self.get_window_index(torch.tensor([[T_feat,H_feat * 2,W_feat * 2]]))
    ...
    def forward(
        ...
        if input_ids is not None and input_ids.shape[1] != 1:  # Prefill stage
            if input_features is not None:
                ...
                audio_features, attn_mean_a = self.get_audio_features(
                    input_features,
        ...
        if pixel_values_videos is not None and audio_features is not None and video_embeds is not None and prune_need is not None:
            if prune_need['args'].prune == True:
                from OmniSelect import omniselect
                print("Using OmniSelect Inference:....")
                with torch.no_grad():
                    device = inputs_embeds.device
                    
                    feat_v = video_embeds  # [N, D]
                    feat_v = F.normalize(feat_v, dim=-1)
                    
                    global_mask = omniselect(
                        video_grid_thw,
                        inputs_embeds,
                        audio_features,
                        video_embeds,
                    ...
                    cnt = 0
                    # for i in global_mask:
                    #     if not i :
                    #         cnt+=1
                    # print(f"actual token pruing ratio:{cnt / len(global_mask)}")
